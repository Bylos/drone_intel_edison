...
void ahrs_BetaUpdate(GyroscopeSensorData gyro) {
	...
	float angularVelocity = sqrt(gyro.GetX()*gyro.GetX()+gyro.GetY()*gyro.GetY()+gyro.GetZ()*gyro.GetZ());
 ...
 */
void ahrs_Madgwick2015(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float deltat)
{
  float norm;
 ...
 */
void ahrs_MadgwickIMU(float ax, float ay, float az, float gx, float gy, float gz, float deltat) {
	float norm;
	float s1, s2, s3, s4;
	float qDot1, qDot2, qDot3, qDot4;
...
AttitudeData ahrs_Quaternion2Euler(void) {
	AttitudeData angles;
	angles.Set(
		atan2(2.0f * (q1 * q2 + q3 * q4), q1 * q1 - q2 * q2 - q3 * q3 + q4 * q4)*180.0f / 3.14159265359f,
		-asin(2.0f * (q2 * q4 - q1 * q3))*180.0f / 3.14159265359f,
		atan2(2.0f * (q2 * q3 + q1 * q4), q1 * q1 + q2 * q2 - q3 * q3 - q4 * q4)*180.0f / 3.14159265359f);
	return angles;
}
...
int ahrs_init(InertialData data, ahrs_filter_t filter) {
	...
	case AHRS_MADGWICK_2015:
		ahrs_Madgwick2015(
				...
				gyro.GetX()*3.14159265359f/180.0f, gyro.GetY()*3.14159265359f/180.0f, gyro.GetZ()*3.14159265359f/180.0f,
				magn.GetX(), magn.GetY(), magn.GetZ(),
				AHRS_INIT_UPDATE_PERIOD);
		break;
